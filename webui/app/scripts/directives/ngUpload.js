...
angular.module('ngUpload', [])
    ...
    // extend it used to link customized element to instead input[type='file']
    .directive('uploadFileInputLink', ["$parse", "$log", function($parse, $log) {
        return {
            restrict: 'AC',
            link: function(scope, element, attrs) {
                element.bind('click', function($event) {
                    var linkedTargetFileControlSelector = attrs["uploadFileInputLink"];
                    var $targetFileControl = angular.element(document.querySelector(linkedTargetFileControlSelector));
                    if ($targetFileControl && $targetFileControl.length) {
                        return $targetFileControl.triggerHandler("click");
                    }
                    // $log.debug("event: ", attrs["uploadFileInputLink"]);
                });
            }
        };
    }])
