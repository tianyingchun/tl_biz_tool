app.controller("PictureBindCtrl", ["$scope", "$log", "PictureService", "statusEnum", "FileService", "regexRules", "DialogService",
    function ($scope, $log, PictureService, statusEnum, FileService, regexRules, DialogService) {
         ...
         */
        function bindPictureAttr(picture) {
        	var data = {
        		url: picture.url,
                sourceDir: $scope.sourceDir || "",
                syncedToDir: $scope.syncedToDir || "",
        	}
			initStatus(picture);
        	var promise = PictureService.bindPictureAttr(data);
        	return promise;
        };
        ...
        this.handle = function (item) {
            if (item.url && item.url.length > 0) {
                ...
                var promise = bindPictureAttr(item);
        ...
        this.doBatch = function () {
            ...
            if ($scope.finalList && $scope.finalList.length > 0) {
                ...
                DialogService.showAlertDialog("没有可以处理的!");
                $scope.doingBatch = false;
            }
        };
